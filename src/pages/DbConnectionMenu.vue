...
import {Database, defaultDatabase, defaultDbUser} from "../lib/db.ts";
...
const connectToDatabase = async () => {
  const host = dbHostInput.value?.input.value;
  const user = dbUserInput.value?.input.value;
  const password = dbPasswordInput.value?.input.value;
  const database = dbDatabaseInput.value?.input.value;

  if (host !== undefined && host.trim().length > 0 &&
      user !== undefined && user.trim().length > 0 &&
      password !== undefined && password.trim().length > 0 &&
      database !== undefined && database.trim().length > 0
  ) {
...
<template>
  <div class="flex flex-col items-center justify-center gap-4">
    ...
    <div class="relative text-neutral-700 focus-within:text-red-900 transition-colors duration-200">
    ...
    <button
        class="outline-red-800 bg-red-800 hover:outline-red-900 hover:bg-red-900 transition duration-200 rounded-full px-3 h-8 text-white hover:text-neutral-200 cursor-pointer disabled:cursor-default disabled:bg-black/75 disabled:text-white"
